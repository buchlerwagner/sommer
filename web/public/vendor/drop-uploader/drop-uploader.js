(function($) {
    jQuery.fn.drop_uploader = function(options) {
        options = $.extend({
            ...
            uploader_icon: '<i class="fa fa-cloud-upload-alt"></i>',
            file_icon: '<i class="far fa-file"></i>',
        ...
        this.each(function(i, val) {
                ...
                if(existing_files.length > 0) {
                    ...
                    for (var fi = 0; fi < existing_files.length; fi++) {
                        ...
                        if(existing_files_removable) {
                            // Add delete button
                            var du_delete_button = $('<i class="far fa-trash-alt action-delete" data-fileid="' + existing_files[fi].id + '"></i>').hide();
                            if(layout == "thumbnails") {
            ...
            drop_zone[0].ondragend = function(event) {
                drop_zone.removeClass('hover');
                return false;
            };
            ...
            function preview_existing_file(thumb, type, i) {
                ...
                if(thumb != '') {
                    ...
                    // Add Icon
                    if(type == 'video') {
                        $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail').html('<i class="far fa-file-video"></i>');
                    } else if(type == 'audio') {
                        $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail').html('<i class="far fa-file-audio"></i>');
                    } else {
                        $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail').html('<i class="far fa-file"></i>');
                    }
                }
            ...
            function file_upload_ajax(file,i) {
                ...
                xhr.addEventListener('load', function(e) {
                    ...
                    if(response.success) {
                        if(parseInt(response.ctid)){
                            $('#msg-ctid').val(response.ctid);
                        }
                        if(parseInt(response.cmid)){
                            $('#msg-cmid').val(response.cmid);
                        }
                        ...
                        if(layout == "table"){
                            ...
                            if(response.table) {
                                tables.reload(response.table, response.tableKeyFields);
                            }
                        }else {
                            ...
                            if (file.type.match('image/*')) {
                                ...
                                $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail').append('<i class="far fa-file-video"></i>');
                                $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail i').hide().delay(500).fadeIn("slow");
                            } else if (file.type.match('audio/*')) {
                                $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail').append('<i class="far fa-file-audio"></i>');
                                $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail i').hide().delay(500).fadeIn("slow");
                            } else {
                                $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail').append('<i class="far fa-file"></i>');
                                $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail i').hide().delay(500).fadeIn("slow");
                            }
                            // Add delete button
                            var du_delete_button = $('<i class="far fa-trash-alt action-delete" data-fileid="' + response.file_id + '"></i>').hide();
                            if (layout == "thumbnails") {
                ...
                var fdata = new FormData(cur_form[0]);
